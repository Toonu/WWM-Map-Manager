...
public class Mesh2DColliderMaker : MonoBehaviour {
	...
	[CustomEditor(typeof(Mesh2DColliderMaker))]
	public class Mesh2DColliderMakerEditor : Editor {
		public override void OnInspectorGUI() {
			DrawDefaultInspector();

			Mesh2DColliderMaker maker = (Mesh2DColliderMaker)target;

			if (GUILayout.Button("Draw")) {
				maker.CreatePolygon2DColliderPoints();
			}
		}
	}
#endif
	...
	static Vector2[] CoordinatesCleaned(List<Vector2> coordinates) {
		List<Vector2> coordinatesCleaned = new() {
			coordinates[0]
		};
